...
class GraphQLService extends GetxService {
  ...
  Future<Map<String, dynamic>> createEquipmentRepairReport(
    ...
    try {
      ...
      final result = await mutate(createEquipmentRepairReportMutation, variables: variables);

      if (result.hasException) {
        print('[GraphQL] Error creating equipment repair report: ${result.exception}');
        throw Exception(result.exception.toString());
      }
      ...
      print('[GraphQL] Equipment repair report created successfully with ID: ${data['id']}');
      return data;
    } catch (e) {
  ...
  Future<List<Map<String, dynamic>>> fetchEquipmentRepairReports({
    ...
    try {
      ...
      if (result.hasException) {
        print('[GraphQL] Error fetching my equipment repair reports: ${result.exception}');
        throw Exception(result.exception.toString());
      }
